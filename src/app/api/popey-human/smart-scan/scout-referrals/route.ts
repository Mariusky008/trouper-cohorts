...
export async function GET() {
  ...
  const { data: referrals, error } = await supabaseAdmin
    ...
    .select("id,scout_id,contact_name,project_type,status,created_at")
  ...
  if (scoutIds.length > 0) {
    ...
    scoutNameById = new Map(
      ((scouts || []) as Array<{ id: string; first_name: string | null; last_name: string | null; email: string | null; phone: string | null }>).map((item) => {
  ...
  return NextResponse.json({
    referrals: ((referrals || []) as Array<{ id: string; scout_id: string; contact_name: string; project_type: string | null; status: string; created_at: string }>).map((row) => ({
      ...row,
