import { NextResponse } from "next/server";
import twilio from "twilio";
import { processTwilioVoiceWebhook } from "@/lib/actions/voice-twilio";
import { voiceTwilioConfig } from "@/lib/popey-human/voice-twilio-config";

export const dynamic = "force-dynamic";

function normalizeParams(formData: FormData) {
  const entries = Array.from(formData.entries());
  return Object.fromEntries(entries.map(([key, value]) => [key, String(value || "")]));
}

export async function GET() {
  return NextResponse.json({
    success: true,
    message: "Webhook Twilio Voice prêt.",
  });
}

export async function POST(request: Request) {
  const formData = await request.formData().catch(() => null);
  if (!formData) {
    return NextResponse.json({ success: false, error: "Payload webhook Twilio invalide." }, { status: 400 });
  }
  const params = normalizeParams(formData);

  if (voiceTwilioConfig.validateWebhookSignature && voiceTwilioConfig.authToken) {
    const signature = request.headers.get("x-twilio-signature") || "";
    const expectedUrl = voiceTwilioConfig.statusCallbackUrl || request.url;
    const isValid = twilio.validateRequest(voiceTwilioConfig.authToken, signature, expectedUrl, params);
    if (!isValid) {
      return NextResponse.json({ success: false, error: "Signature Twilio invalide." }, { status: 403 });
    }
  }

  const result = await processTwilioVoiceWebhook(params);
  if (!result.success) {
    return NextResponse.json({ success: false, error: "Traitement webhook Twilio en erreur." }, { status: 400 });
  }
  return NextResponse.json(result);
}

