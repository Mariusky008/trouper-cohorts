...
export async function GET(request: Request, context: RouteContext) {
  ...
  const { data: site, error } = await supabase
    ...
    .select("slug,status,preview_storage_prefix,preview_token")
  ...
  const previewPrefix = String(site.preview_storage_prefix || "").trim();
  if (!previewPrefix) return new Response("Not found", { status: 404 });

  const assetPath = `${previewPrefix.replace(/\/+$/g, "")}/assets/${rel}`;
