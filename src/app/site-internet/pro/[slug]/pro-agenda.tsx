...
type DayState = { open: boolean; windows: Win[] }; // plusieurs plages/jour (9h-12h + 14h-18h…)
...
export function ProAgenda({
  ...
  const apply = (j: Record<string, unknown>) => {
    ...
    const next: Record<number, DayState> = Object.fromEntries(ORDER.map((wd) => [wd, { open: false, windows: [] as Win[] }]));
    if (Array.isArray(j.windows)) {
      for (const w of j.windows as Array<Record<string, unknown>>) {
        const wd = Number(w.weekday);
        if (wd in next) { next[wd].open = true; next[wd].windows.push({ start: toTime(Number(w.start_min)), end: toTime(Number(w.end_min)) }); }
      }
    }
    // Toujours au moins une plage éditable par jour (même fermé).
    for (const wd of ORDER) {
      next[wd].windows.sort((a, b) => toMin(a.start) - toMin(b.start));
      if (next[wd].windows.length === 0) next[wd].windows = [{ start: "09:00", end: "18:00" }];
    }
    setDayState(next);
  ...
  const setOpen = (wd: number, open: boolean) => setDayState((s) => ({ ...s, [wd]: { ...s[wd], open } }));
  const setWin = (wd: number, idx: number, patch: Partial<Win>) =>
    setDayState((s) => ({ ...s, [wd]: { ...s[wd], windows: s[wd].windows.map((w, i) => (i === idx ? { ...w, ...patch } : w)) } }));
  ...
  const save = async () => {
    ...
    for (const wd of ORDER) {
      if (!dayState[wd].open) continue;
      for (const w of dayState[wd].windows) {
        if (toMin(w.end) > toMin(w.start)) windows.push({ weekday: wd, start_min: toMin(w.start), end_min: toMin(w.end) });
      }
    }
    try {
    ...
    <>
      <style
        dangerouslySetInnerHTML={{
          ...
          .pro .agenda .day{display:flex;flex-direction:column;gap:8px;border:1px solid var(--hair);border-radius:11px;padding:10px 11px;background:#fff;}
          ...
          {ORDER.map((wd) => {
            ...
            return (
              <div key={wd} className={`day${d.open ? "" : " off"}`}>
                <div className="dhead">
                  <input type="checkbox" checked={d.open} onChange={(e) => setOpen(wd, e.target.checked)} aria-label={NAMES[wd]} />
                  <span className="nm">{NAMES[wd]}</span>
                  {d.open && d.windows.length < 4 && (
                    <button type="button" className="addwin" onClick={() => addWin(wd)}>＋ plage</button>
                  )}
                </div>
                {d.open && (
                  <div className="wins">
