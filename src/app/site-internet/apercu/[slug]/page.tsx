...
// Une vraie mini-vitrine mobile pour le commerçant — moderne, responsive, avec
// un bouton Appeler en un geste : elle RÉSOUT le verdict du recto. Un bandeau
// discret rappelle que c'est une maquette préparée par Marius. Le scan est tracké.
...
export default async function ApercuMaquette({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data } = await supabase
    ...
    .select("id, business_name, city, activite, google_rating, google_reviews, diagnostic")
    .eq("slug", slug)
    .eq("channel", "letter")
    .maybeSingle();
  ...
  // 3 "services" génériques dérivés de l'activité (illustratif, éditable ensuite).
  const services = ["Nos prestations", "Prendre rendez-vous", "Nous trouver"];
    ...
    <main className="mq">
      <style
        dangerouslySetInnerHTML={{
          ...
          .mq .hero{background:var(--dk);color:var(--cr);padding:34px 22px 26px;}
          .mq .kicker{font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:var(--soft);}
          ...
          .mq .card{aspect-ratio:1;border-radius:12px;background:linear-gradient(160deg,#F3F1EC,#E7E4DC);border:1px solid #E4E1D9;}
          ...
          .mq .lead-wrap{padding:24px 22px 8px;}
      ...
      <section className="sec">
        <h2>Un aperçu</h2>
        <div className="cards"><div className="card" /><div className="card" /><div className="card" /></div>
      </section>
      ...
      <section className="sec">
        <h2>Nous trouver</h2>
        <p style={{ margin: "0 0 12px", color: "#6E6E64", fontSize: 14.5 }}>{ville ? `${ville}` : ""}</p>
        <a href={mapsHref} style={{ color: "#14140F", fontWeight: 600, textDecoration: "underline" }}>Voir l&apos;itinéraire →</a>
      </section>

      <div className="lead-wrap">
        <h2 style={{ fontFamily: "Georgia, serif", fontSize: 20, margin: "0 0 6px" }}>Cette maquette vous plaît ?</h2>
        <p style={{ color: "#6E6E64", fontSize: 14, margin: "0 0 14px" }}>Je la mets en ligne pour de vrai. Laissez votre numéro ou appelez-moi — c&apos;est Marius.</p>
        <LeadForm slug={slug} />
      </div>

      <div className="maq-note">
        <b>C&apos;est une maquette, faite pour {nom}.</b><br />
        Elle vous plaît ? On la met en ligne en 72 h. Sinon, ça s&apos;arrête là — sans frais.
