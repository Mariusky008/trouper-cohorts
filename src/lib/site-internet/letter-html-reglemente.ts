// LA LETTRE DES PROFESSIONS RÉGLEMENTÉES — santé et droit.
//
// C'est l'ANCIENNE lettre, celle qui vendait un site à 690 €. Elle n'a pas été
// remplacée par erreur : la nouvelle annonce Le Direct, c'est-à-dire la
// publication d'annonces commerciales dans un fil public — incompatible avec
// une profession réglementée, sans version adoucie possible.
//
// Mais un diététicien a toujours besoin d'un site, et il peut toujours le
// payer. Refuser d'imprimer QUOI QUE CE SOIT pour lui revenait à confondre
// « il ne peut pas faire de promotion » avec « on n'a rien à lui proposer ».
//
// Deux lettres, deux audiences, deux fichiers :
//   · `letter-html.ts`             → commerces, Le Direct, gratuit ;
//   · celui-ci                     → santé et droit, site seul, payant.
//
// Elles ne partagent volontairement rien. Le message, l'argument et le
// vocabulaire diffèrent de bout en bout : les factoriser produirait un gabarit
// à conditions dont plus personne n'oserait toucher une ligne.
...
export function readLetterStylesReglementee(): string {
...
export async function composeLettreReglementee(input: {
