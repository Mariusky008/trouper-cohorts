...
export const whatsappSendTemplateSchema = z
  .object({
    ...
    metadata: z
      .record(z.string().trim().min(1).max(64), z.union([z.string().max(220), z.number(), z.boolean(), z.null()]))
      .optional(),
  })
  .strict();
...
export const whatsappTemplateSubmitSchema = z
  .object({
    ...
    quick_replies: z.array(nonEmpty(40)).max(3).default(["Oui, avec plaisir", "Pas pour le moment"]),
    owner_member_id: z.string().uuid().optional(),
  })
  .strict();
